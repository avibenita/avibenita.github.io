...
(function () {
  ...
  function ensureStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = [
      '.ws-modal-backdrop{position:fixed;inset:0;z-index:9000;background:rgba(215,220,228,.85);backdrop-filter:blur(14px);display:block;opacity:0;pointer-events:none;transition:opacity .25s ease;overflow:hidden}',
      '.ws-modal-backdrop.open{opacity:1;pointer-events:all}',
      '.ws-modal{position:absolute;inset:12px;width:auto;height:auto;max-height:none;padding:10px 48px 10px 10px;box-sizing:border-box;background:#e8eaee;border:1px solid rgba(15,23,42,.08);border-radius:16px;overflow:hidden;box-shadow:0 30px 90px rgba(15,23,42,.30),inset 0 1px 0 rgba(255,255,255,.7);display:flex;flex-direction:column;gap:10px}',
      '.ws-modal-title{display:none}',
      '.ws-modal-close{position:absolute;top:12px;right:12px;z-index:6;width:38px;height:38px;border-radius:12px;border:1px solid rgba(15,23,42,.16);background:#0f172a;color:rgba(255,255,255,.86);cursor:pointer;font-size:1rem;display:flex;align-items:center;justify-content:center;transition:all .15s;font-family:inherit}',
  ...
  function boot() {
    ...
    function fitFrame() {
      ...
      fitRetries = 0;
      // Fill the visible frame so Visualisation / Data Manipulation / results
      // dialogs use the whole window instead of a short 1280×800 letterbox.
      clearScale();
    }
