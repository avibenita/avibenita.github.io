...
(function (root, factory) {
  ...
  function scanQuality(headers, rows, options) {
    ...
    headers.forEach(function (h, i) {
      if (h == null || String(h).trim() === '') blankHeaders.push(i);
    });
    if (blankHeaders.length) {
      issues.push({
        id: 'blank-headers',
        variable: blankHeaders.map(function (i) { return 'Column ' + excelColLetter(i); }).join(', '),
        issue: 'No header detected',
        affected: blankHeaders.length,
        severity: 'recommended',
        suggested: 'Name these columns, or omit them from the prepared worksheet. The source sheet is not changed.',
        kind: 'blank_headers',
        inspect: blankHeaders.map(function (i) { return { row: 1, value: 'Column ' + excelColLetter(i) + ' has no header' }; })
      });
    }
  ...
  function describeStep(step, result) {
    ...
    switch (step.type) {
      ...
      case 'longToWide':
        return 'Reshaped ' + (step.timeVar || 'occasion') + ' / ' + (step.valueVar || 'value') + ' into wide format';
      case 'transpose':
        return 'Transposed the table (first column became the new headers)';
      case 'dropVariables':
  ...
  function occasionName(v) {
    if (isBlank(v)) return 'Blank';
    return String(v).replace(/[:\\/?*\[\]]/g, '_');
  }

  function applyLongToWide(ds, step) {
    var idIdx = headerIndex(ds.headers, step.idVar);
    var timeIdx = headerIndex(ds.headers, step.timeVar);
    var valueIdx = headerIndex(ds.headers, step.valueVar);
    if (idIdx < 0) return { error: 'Select the subject / ID column.', casesAffected: 0 };
    if (timeIdx < 0) return { error: 'Select the occasion column (values become new column names).', casesAffected: 0 };
    if (valueIdx < 0) return { error: 'Select the value column to spread.', casesAffected: 0 };
    if (idIdx === timeIdx || idIdx === valueIdx || timeIdx === valueIdx) {
      return { error: 'ID, occasion, and value must be three different columns.', casesAffected: 0 };
    }
    var stubIdx = ds.headers.map(function (_h, i) { return i; }).filter(function (i) {
      return i !== idIdx && i !== timeIdx && i !== valueIdx;
    });
    var baseHeaders = [ds.headers[idIdx]].concat(stubIdx.map(function (i) { return ds.headers[i]; }));
    var timeNames = [];
    var timeKeyToName = {};
    ds.rows.forEach(function (row) {
      var raw = occasionName(row[timeIdx]);
      var key = raw.toLowerCase();
      if (timeKeyToName[key]) return;
      var name = uniqueName(baseHeaders.concat(timeNames), raw);
      timeKeyToName[key] = name;
      timeNames.push(name);
    });
    if (!timeNames.length) return { error: 'The occasion column has no values to spread.', casesAffected: 0 };
    var groups = [];
    var groupMap = {};
    ds.rows.forEach(function (row) {
      var id = cellKey(row[idIdx]);
      if (!groupMap[id]) {
        groupMap[id] = { id: row[idIdx], stub: stubIdx.map(function (i) { return row[i]; }), vals: {} };
        groups.push(groupMap[id]);
      }
      var tname = timeKeyToName[occasionName(row[timeIdx]).toLowerCase()];
      if (tname) groupMap[id].vals[tname] = row[valueIdx];
    });
    var newHeaders = baseHeaders.concat(timeNames);
    var newRows = groups.map(function (g) {
      return [g.id].concat(g.stub).concat(timeNames.map(function (t) {
        return Object.prototype.hasOwnProperty.call(g.vals, t) ? g.vals[t] : MISSING;
      }));
    });
    var nOld = ds.rows.length;
    ds.headers = newHeaders;
    ds.rows = newRows;
    ds.structural = true;
    ds.newVariables = ds.newVariables.concat(timeNames);
    return { casesAffected: newRows.length, outputRows: newRows.length, inputRows: nOld };
  }

  function applyTranspose(ds) {
    if (ds.headers.length < 2) return { error: 'Need at least two columns to transpose.', casesAffected: 0 };
    if (!ds.rows.length) return { error: 'Need at least one data row to transpose.', casesAffected: 0 };
    var firstName = ds.headers[0];
    var colNames = [];
    ds.rows.forEach(function (row, i) {
      var raw = isBlank(row[0]) ? ('Col_' + (i + 1)) : String(row[0]).replace(/[:\\/?*\[\]]/g, '_');
      colNames.push(uniqueName([firstName].concat(colNames), raw));
    });
    var newHeaders = [firstName].concat(colNames);
    var newRows = [];
    for (var c = 1; c < ds.headers.length; c++) {
      var nr = [ds.headers[c]];
      for (var r = 0; r < ds.rows.length; r++) nr.push(ds.rows[r][c]);
      newRows.push(nr);
    }
    var nOld = ds.rows.length;
    ds.headers = newHeaders;
  ...
  var OPERATORS = {
    ...
    longToWide: applyLongToWide,
    transpose: applyTranspose,
  ...
  function validateStep(step, headers) {
    ...
    if (step.type === 'longToWide') {
      if (!step.idVar) return { ok: false, error: 'Select the subject / ID column.' };
      if (!step.timeVar) return { ok: false, error: 'Select the occasion column.' };
      if (!step.valueVar) return { ok: false, error: 'Select the value column.' };
      if (step.idVar === step.timeVar || step.idVar === step.valueVar || step.timeVar === step.valueVar) {
        return { ok: false, error: 'ID, occasion, and value must be three different columns.' };
      }
      if (headerIndex(headers, step.idVar) < 0 || headerIndex(headers, step.timeVar) < 0 || headerIndex(headers, step.valueVar) < 0) {
        return { ok: false, error: 'ID, occasion, and value must be columns in the current dataset.' };
      }
    }
    if (step.type === 'transpose') {
      if ((headers || []).length < 2) return { ok: false, error: 'Need at least two columns to transpose.' };
    }
  ...
  return {
    ...
    AVAILABLE_OPERATIONS: [
      { id: 'defineMissing', category: 'variables', label: 'Missing-value codes' },
      { id: 'dropVariables', category: 'variables', label: 'Keep / remove' },
      { id: 'recode', category: 'variables', label: 'Recode values' },
      { id: 'compute', category: 'variables', label: 'Compute variable' },
      { id: 'reverseScore', category: 'variables', label: 'Reverse-score items' },
      { id: 'composite', category: 'variables', label: 'Composite score' },
      { id: 'harmonize', category: 'variables', label: 'Harmonize labels' },
      { id: 'filter', category: 'cases', label: 'Filter cases' },
      { id: 'flagDuplicates', category: 'cases', label: 'Flag duplicates' },
      { id: 'wideToLong', category: 'structure', label: 'Wide to long' },
      { id: 'longToWide', category: 'structure', label: 'Long to wide' },
      { id: 'transpose', category: 'structure', label: 'Transpose' }
    ]
  };
});
