...
(function (root) {
  ...
  var EXAMPLES = {
    defineMissing: {
      ...
      explain: 'Code 99 represents “no answer,” so it becomes missing in the prepared worksheet.',
      headers: ['ID', 'Score'],
      before: [['A', '99'], ['B', '12']],
      after: [['A', ''], ['B', '12']],
      hitBefore: ['0,1'],
  ...
  function mount() {
    ...
    style.textContent = [
      ...
      '.ex-cmp{display:grid;grid-template-columns:1fr 1fr;gap:8px;}',
      '.ex-cmp .ex-phase{margin:0 0 4px;}',
      '.ex-ov:not(.staged) .ex-nav{display:none;}',
      '.ex-ov:not(.staged) #exBack,.ex-ov:not(.staged) #exNext{display:none;}',
      '.ex-ov.staged #exDone{display:none;}',
    ].join('');
    ...
    ov.innerHTML =
      '<div class="ex-card" role="dialog" aria-modal="true">' +
        '<div class="ex-head"><div><h3 id="exTitle"></h3><p id="exSub">Illustration only—your data will not be changed</p></div>' +
        '<button type="button" class="ex-x" id="exClose" aria-label="Close">×</button></div>' +
        '<div class="ex-body">' +
          '<div class="ex-phase ex-nav">' +
        ...
        '<div class="ex-foot">' +
          ...
          '<button type="button" class="ex-btn ghost" id="exDone">Close example</button>' +
        '</div>' +
      '</div>';
  ...
  var STAGED = { wideToLong: true, longToWide: true, composite: true };

  function compactHtml(ex) {
    var afterHeaders = ex.afterHeaders || ex.headers;
    return '<div class="ex-cmp">' +
      '<div><div class="ex-phase">Before</div>' + tableHtml(ex.headers, ex.before, ex.hitBefore) + '</div>' +
      '<div><div class="ex-phase">After</div>' + tableHtml(afterHeaders, ex.after, ex.hitAfter) + '</div>' +
      '</div>';
  }
  ...
  function play(type) {
    ...
    currentType = type;
    var staged = !!STAGED[type];
    var ov = $('prepExOverlay');
    ov.setAttribute('data-op', type);
    ov.classList.toggle('staged', staged);
    ov.classList.add('on');
    $('exSkipAuto') && ($('exSkipAuto').checked = skipAuto());
    $('exTitle').textContent = 'Example: ' + EXAMPLES[type].title;
    $('exSub').textContent = 'Illustration only—your data will not be changed';
    if (staged) {
      go(0);
      return;
    }
    var ex = EXAMPLES[type];
    $('exNote').textContent = ex.explain || ex.afterNote || ex.howNote || '';
    $('exHowBox').style.display = 'none';
    $('exTable').innerHTML = compactHtml(ex);
  }
