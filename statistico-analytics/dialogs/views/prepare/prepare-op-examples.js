/**
 * Animated before / how / after examples for Prepare Dataset operations.
 */
(function (root) {
  'use strict';
  ...
  function mount() {
    ...
    style.textContent = [
      ...
      '.ex-dot{width:7px;height:7px;border-radius:99px;background:#334155;}',
      ...
      '.ex-hit{animation:exPulse .9s ease 2;}',
      '.ex-miss{font-style:italic;color:#f97316;font-weight:700;}',
      '.ex-howbox{margin:8px 0;padding:8px 10px;border-radius:10px;border:1px dashed var(--prep-acc,#14b8a6);color:var(--prep-acc,#14b8a6);font-size:12px;font-weight:700;text-align:center;animation:exPop .45s ease;}',
      ...
      '@keyframes exPulse{0%,100%{box-shadow:inset 0 0 0 0 rgba(20,184,166,0);}50%{box-shadow:inset 0 0 0 2px rgba(20,184,166,.7);}}',
      '@keyframes exPop{from{transform:scale(.92);opacity:0;}to{transform:scale(1);opacity:1;}}'
    ].join('');
    ...
    ov.innerHTML =
      '<div class="ex-card" role="dialog" aria-modal="true">' +
        ...
        '<div class="ex-body">' +
          '<div class="ex-phase"><span class="ex-dot" id="exDot0"></span><span class="ex-dot" id="exDot1"></span><span class="ex-dot" id="exDot2"></span>' +
        ...
        '</div>' +
        '<div class="ex-foot">' +
          '<button type="button" class="ex-btn ghost" id="exReplay">Replay</button>' +
          '<button type="button" class="ex-btn pri" id="exDone">Got it</button>' +
    ...
    $('exDone').onclick = hide;
    $('exReplay').onclick = function () { play($('prepExOverlay').getAttribute('data-op'), true); };
  ...
  function play(type, fromReplay) {
    ...
    timer = setTimeout(function () {
      showHow();
      timer = setTimeout(showAfter, 2200);
    }, fromReplay ? 700 : 900);
  }
