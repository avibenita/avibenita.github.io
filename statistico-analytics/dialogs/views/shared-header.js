...
const StatisticoHeader = {
  ...
  _bindCollapsibleSidebarGroups(nav) {
    ...
    groups.forEach((group) => {
      ...
      group.querySelectorAll('.sb-collapse-chevron').forEach((el) => el.remove());
  ...
  _getExportDefaultSectionIds(sections) {
    ...
    return sections.map((s) => String(s.id));
  },
  ...
  _pickReportSections(sections, onConfirm, options = {}) {
    ...
    const sectionListHtml = sections.map((s) => {
      ...
      return `
        <label class="st-export-check-row">
          <input type="checkbox" class="st-export-check" data-section-id="${esc(sid)}" ${checked} />
    ...
    const optionsExpanded = !hideCover && initialCover.optionsExpanded === true;
  ...
  _ensurePlainTabUnderlineStyles() {
    ...
    style.textContent = [
      ...
      '  grid-template-areas: "kicker ." "tabs desc" !important;',
      ...
      '}',
      '.view-switcher-desc,',
      '.view-switcher-row .uni-view-caption {',
      ...
      '  background: linear-gradient(180deg, rgba(42, 54, 82, 0.88) 0%, rgba(18, 26, 42, 0.82) 100%) !important;',
      '  background-image: linear-gradient(180deg, rgba(42, 54, 82, 0.88) 0%, rgba(18, 26, 42, 0.82) 100%) !important;',
      ...
      '  opacity: 1 !important;',
      '  filter: none !important;',
      '  box-shadow: 0 0 14px rgba(124, 92, 255, 0.42), 0 0 28px rgba(124, 92, 255, 0.18), inset 0 0 0 1px rgba(167, 139, 250, 0.22) !important;',
      '  transform: none !important;',
      '}',
      '.uni-view-tabs.ws-mode-bar--connected .ws-mode-tab > :not(.view-switcher-glow),',
      '.view-switcher-bar.ws-mode-bar--connected .ws-mode-tab > :not(.view-switcher-glow) {',
      '  position: relative !important;',
      '  z-index: 1 !important;',
      '}',
      '.uni-view-tabs.ws-mode-bar--connected .ws-mode-tab .view-switcher-glow,',
      '.view-switcher-bar.ws-mode-bar--connected .ws-mode-tab .view-switcher-glow {',
      '  display: none;',
      '  position: absolute;',
      '  inset: 0;',
      ...
      '}',
      '.uni-view-tabs.ws-mode-bar--connected .ws-mode-tab.active .view-switcher-glow::before,',
      '.view-switcher-bar.ws-mode-bar--connected .ws-mode-tab.active .view-switcher-glow::before {',
      '  content: "" !important;',
      '  display: block !important;',
      ...
      '  background: conic-gradient(from 0deg, rgba(196, 181, 253, 0.55) 0%, rgba(120, 200, 255, 0.38) 22%, transparent 40%, transparent 100%) !important;',
      '  animation: view-switcher-rotate-glow 8s linear infinite !important;',
      '  mix-blend-mode: screen;',
      '  opacity: 0.95;',
      '  pointer-events: none;',
      '}',
      '.uni-view-tabs.ws-mode-bar--connected .ws-mode-tab.active .view-switcher-glow::after,',
      '.view-switcher-bar.ws-mode-bar--connected .ws-mode-tab.active .view-switcher-glow::after {',
      '  content: "" !important;',
      '  display: block !important;',
      '  position: absolute !important;',
      '  inset: 5px !important;',
      '  border-radius: inherit !important;',
      '  background: radial-gradient(circle at 28% 18%, rgba(255, 255, 255, 0.18) 0%, rgba(167, 139, 250, 0.12) 38%, transparent 72%) !important;',
      '  animation: view-switcher-pulse-inner 4.5s ease-in-out infinite !important;',
      '  pointer-events: none;',
      '}',
      ...
      '  box-shadow: none !important;',
      '}',
      'html[data-theme="light"] .uni-view-tabs.ws-mode-bar--connected .ws-mode-tab.active .view-switcher-glow::before,',
      'html[data-theme="light"] .view-switcher-bar.ws-mode-bar--connected .ws-mode-tab.active .view-switcher-glow::before {',
      '  display: none !important;',
      '}',
      ...
      '  color: #ffffff !important;',
      '}',
      '@keyframes view-switcher-rotate-glow {',
      '  from { transform: translate(-50%, -50%) rotate(0deg); }',
      '  to { transform: translate(-50%, -50%) rotate(360deg); }',
      '}',
      '@keyframes view-switcher-pulse-inner {',
...
REPORT: [one polished paragraph suitable for a report]`;
  ...
  _genericModuleControlsDoc(key) {
    ...
    const docs = {
      ...
      'meta-bias':
        'This combines Egger\'s regression test (intercept, SE of intercept, t(df), p, n) with a funnel plot of log-ratio effect vs SE of log effect (for RR/OR), including the pooled log-effect line and Pseudo-95% limits (μ̂ ± 1.96×SE; expected funnel limits, not a CI for the pooled effect). Describe whether points sit inside the funnel and whether Egger suggests small-study effects. Stress that asymmetry is not only publication bias, and with k < 10 results are unstable / limited reliability. Do not re-walk the full pooled-effect narrative from Summary.',
      'meta-studies':
  ...
  _buildGenericInsightGuidePrompt(viewKey) {
    ...
    const chartGuards = {
      ...
      'correlation-by-group-similarity':
        'This is Similarity Profile™: a homogeneity gauge, an all-group correlation profile, and a pairwise table of 0–100 descriptive scores (pattern / strength / sign) plus a pair profile. Open Congruence scatter from the selected pair. Group labels use col="level" (n=N). Focus READING on practical similarity, not significance. Cite Pattern / Strength / Sign when a pair is mixed. Do not call this a published coefficient — it is Statistico\'s Similarity Profile™. Use Very similar / Mostly similar / Mixed similarity / Substantially different.',
      'logistic-predictive-performance-roc-thresholds':
...
${this._localAiReplyFormat()}`;
  ...
  _correlationControlsDoc() {
    return {
      ...
      'correlation-by-group-similarity': 'Similarity Profile™ compares group correlation structures (not Overall) on pattern (Tucker congruence of aligned pairwise r), strength (mean |r|), and sign agreement. Scores are 0–100 descriptive similarity, not a matrix-difference test. The layout is a homogeneity gauge, an all-group correlation profile, and a pairwise table with a three-component profile. Click a table row for the profile; open Congruence scatter from the pair profile, then click a point for that pair’s XY scatter. Group labels use col="level" (n=N).'
    };
  },
