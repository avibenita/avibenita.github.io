...
READING: [1-2 sentences about what the current tab shows, using exact values where available]`;
  ...
  _viewControlsDoc() {
    return {
      ...
      'by-group-stats': `Controls available: (1) Compare Groups / Change — pick the categorical column that splits the numeric variable; (2) Group level checkboxes in the dialog — include or exclude specific levels; (3) Minimum group size — keep only levels with at least n rows; (4) Source row filter (header) — limits which rows enter every group; (5) Grouped / Histograms / Boxplots toggle — overlapping kernel or histogram overlay, separate histogram panels, or one box plot; (6) Kernel / Histogram toggle on the grouped overlay — filled densities or true overlapping bars on a shared value scale; (7) Select checkbox in the statistics table — emphasize that group's series (click again to clear). A chart series or legend item does the same. The table shows per-group N, mean, CI, spread, and shape statistics.`,
