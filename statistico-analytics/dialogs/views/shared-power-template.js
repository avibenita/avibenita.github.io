(function(){
  ...
  function renderAnalysisLayout(container, opts) {
    ...
    container.innerHTML = [
      '<div class="pwstd-shell pwstd-shell--analysis pwstd-shell--fit pwstd-mode-fromN" id="pwstd-shell" data-pwstd-version="20260705e">',
      '  <header class="pwstd-page-header pwstd-page-header--compact">',
      ...
      '      </div>',
      '      <div class="pwstd-task-bar pwstd-task-bar--inline" role="group" aria-label="Power analysis task">',
      ...
      '      </div>',
      '    </div>',
      '    <div class="pwstd-engine pwstd-engine--header"><i class="fa-solid fa-calculator"></i> <span id="' + id(ids,'engineNote','powEngineNote') + '">Exact noncentral F (G*Power fixed regression model) — computed in browser via jStat.</span></div>',
      '  </header>',
      '  <div class="pwstd-summary-band">',
      '    <div class="pwstd-exec pwstd-exec--neutral" id="pwstd-exec-summary">Run a regression model to populate power results.</div>',
      '    <div class="pwstd-metrics-row pwstd-metrics-row--compact">',
      ...
      '        </div>',
      '        <div class="pwstd-curve-legend pwstd-curve-legend--compact">',
      ...
      '          <span><i class="pwstd-legend-line pwstd-legend-line--target"></i> Target</span>',
      '        </div>',
      '      </div>',
      '    </div>',
      '  </div>',
      ...
      '    </div>',
      '  </div>',
      '  <div class="pwstd-band pwstd-band--recommend pwstd-band--fit-hidden" id="' + id(ids,'status','powStatusMessage') + '" hidden>Run analysis to populate power results.</div>',
      '</div>'
    ].join('\n');
