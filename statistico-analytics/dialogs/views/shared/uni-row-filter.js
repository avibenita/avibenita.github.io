...
(function (global) {
  ...
  function rowsEqual(a, b) {
    ...
    for (var i = 0; i < a.length; i++) {
      var ra = Array.isArray(a[i]) ? a[i] : [a[i]];
      var rb = Array.isArray(b[i]) ? b[i] : [b[i]];
      if (ra.length !== rb.length) return false;
      for (var j = 0; j < ra.length; j++) {
        if (cellStr(ra[j]) !== cellStr(rb[j])) return false;
      }
    }
    return true;
  }

  function markPendingState() {
    _hasPendingChanges = !rowsEqual(_filteredRows, _appliedRows);
  }

  function resetWorkingToApplied() {
    _columnFilters = cloneColumnFilters(_appliedColumnFilters);
    _filteredRows = cloneRows(_appliedRows);
  ...
  function applyAllFilters() {
    ...
    if (!activeColumns.length) {
      _filteredRows = cloneRows(_allRows || []);
      markPendingState();
      updateBadge();
    ...
    if (summary) {
      ...
      if (active) {
        summary.textContent = 'Showing ' + showing + ' of ' + total + ' rows from the selected range'
          ...
          + (_hasPendingChanges
            ? ' Click Finish & Apply to freeze this filtered dataset for the module.'
            : ' Click ▼ on a column header to adjust.');
      } else {
        summary.textContent = _hasPendingChanges
          ? 'Preview updated. Click Finish & Apply to freeze this dataset for analysis.'
          : 'Filter rows from the workbook range. Analysis updates after Finish & Apply.';
      }
    }
    var finishBtn = document.getElementById('uniFilterApplyBtn');
    if (finishBtn) {
      finishBtn.disabled = !_hasPendingChanges;
      finishBtn.style.opacity = _hasPendingChanges ? '1' : '.55';
      finishBtn.style.cursor = _hasPendingChanges ? 'pointer' : 'default';
    }
  }
  ...
  function renderFilterTable() {
    ...
    var limitNote = rows.length > displayRows.length
      ...
      : '';
    el.innerHTML = limitNote + '<table class="uni-filt-grid" style="width:max-content;min-width:100%;border-collapse:collapse;font-size:12px;table-layout:auto;">'
      + '<thead><tr>' + headerCells + '</tr></thead>'
      + '<tbody>' + bodyRows + '</tbody></table>';
  }

  function openFilter() {
    resetWorkingToApplied();
    renderFilterTable();
